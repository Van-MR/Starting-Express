<template lang="html">
  ...
  <el-form :model="addForm" class="addForm">
    ...
    <el-form-item label="台词" :label-width="formLabelWidth">
      <el-input v-model="addForm.favorite" auto-complete="off"></el-input>
    </el-form-item>
...
<!-- 修改数据 -->
  ...
  <el-form :model="modifyForm" class="addForm">
    ...
    <el-form-item label="台词" :label-width="formLabelWidth">
      <el-input v-model="modifyForm.favorite" auto-complete="off"></el-input>
    </el-form-item>
...
export default {
  ...
  methods: {
    ...
    addSure: function() {
      ...
      this.$http.post("/api/hero", addObj).then(res => {
          if(res.data.code == '0'){
            this.$message({
              message: "添加成功",
              type: "success",
    ...
    modifySure: function() {
      this.$http.put(`/api/hero/${this.modifyId}`, this.modifyForm, {emulateJSON: true})
                .then(res => {
                    if(res.data.code == '0'){
                      this.$message({
                      message: "修改成功",
    ...
    deleteDate: function(id) {
      ...
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$http.delete(`/api/hero/${deleteId}`).then(res => {
              console.log(res)
              if(res.data.code == '0'){
                this.$message({
                  type: "info",
